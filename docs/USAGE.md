...
Comprehensive guide for using pg2sqlite-rs.
...
The easiest way to install pg2sqlite:
...
# Or just build
cargo build --release
# Binary at: ./target/release/pg2sqlite
```
...
```bash
pg2sqlite file1.yaml file2.yaml file3.yaml
```
...
```bash
pg2sqlite -c .yamllint file.yaml
```
...
pg2sqlite-rs looks for configuration in these locations (in order):
...
```bash
# Use specific config
pg2sqlite -c my-config.yml src/

# Use relaxed preset
pg2sqlite -d relaxed file.yaml

# Colored output
pg2sqlite -f colored file.yaml

# Strict mode (warnings cause exit code 2)
pg2sqlite --strict file.yaml

# List files without linting
pg2sqlite --list-files src/
```
...
```bash
pg2sqlite -f parsable file.yaml
```
...
jobs:
  lint:
    ...
    steps:
      ...
      - name: Install pg2sqlite
        run: cargo install --git https://github.com/hiromaily/pg2sqlite-rs pg2sqlite

      - name: Lint YAML files
        run: pg2sqlite --strict .
```
...
You can use pg2sqlite-rs as a library in your Rust projects:

### Add Dependency

```toml
[dependencies]
pg2sqlite-core = { path = "../pg2sqlite-rs/core" }
# or when published:
# pg2sqlite-core = "0.1"
```
